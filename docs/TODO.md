...
> **状态（2026-07-03，v2.5.0）**：TODO-1~7 全部已解决；数据丢失 BUG 已修复（涟漪改用 CSS Custom Highlight API，零 DOM 突变）；Code Review P1-1 / P2-3 / P2-5 / P3-7 / P3-8 已修复。遗留：P2-4（嵌套块距离 fromIndex+1 fallback，低优先级）+ isScrolling cooldown 回归待复现。
...
## v2.5.0 已解决（涟漪重写 + Code Review 收尾）

| 编号 | 问题 | 解决方式 |
|---|---|---|
| 数据丢失 BUG | 在 `。/？/！` 后继续打字会删除已有内容 | v2.5.0 重写 ripple.ts 改用 CSS Custom Highlight API（零 DOM 突变）。旧 span 包裹用 `range.extractContents()`+`insertNode(span)` 分裂文本节点，SiYuan 的 input/keydown 处理器在突变后重新查选区时选区语义改变 → 光标飘走 → 内容丢失 |
| 涟漪重写 | span 包裹 DOM 突变 → CSS Custom Highlight API | `CSS.highlights.set("zt-sentence-dim", new Highlight(...ranges))` + `::highlight(zt-sentence-dim)`（ripple.ts:147-192）。463→265 行。零 DOM 突变 = 无数据丢失 / 无光标位移。SiYuan `outerHTML` 交换使 range 失效，下次 `selectionchange` → `applyRipple` 用新文本节点重建（自愈） |
| P1-1 | 滚轮 / 失焦 / 点击后 ripple opacity 残留 | ripple 订阅 inputMode，`focusActive→false` 时 `clearAll()`（ripple.ts:279-281） |
| P2-3 | smoothScroll 死合并路径 | 移除 `pendingScroll` 合并分支，改 drop 策略（TODO-5 设计）；`cancelAnimationFrame` 作防御性保留（typewriter.ts:127-130） |
| P2-5 | 死 `isEmbedBlock` + `EMBED_MULTIPLIER` 代码 | `isRippleTargetBlock` 在乘数前已排除 embed 类型，移除 ripple.ts 内 `isEmbedBlock` 与乘数应用；`EMBED_MULTIPLIER` 保留在 config.ts 作设计文档，使用点有 TODO（ripple.ts:218-221） |
| P3-7 | FLIP 过渡 style 在被跳过元素上残留 | 新增 `lastFLIPElements` 跟踪，下一轮入口清理残留 inline transition/transform（typewriter.ts:15,51-57） |
| P3-8 | 非 ripple-target 的 currentBlock opacity=1 泄漏 | `if (isRippleTargetBlock(currentBlock)) currentBlock.style.opacity = "1"` 守门（ripple.ts:225） |
...
### TODO-6: typewriter.ts — 首字符不滚动 ✅ 已解决

**解决 commit**：`259a965` — 空块守卫在 return 前清空 `lastCheckRect = null`（typewriter.ts:212-230，关键行 227），使下次首字符 `checkAndScroll` 的 `prevY=undefined`，绕过 `|firstCharY − emptyBlockY| > 3` 的 defer 级联，滚动不再丢失。
**优先级**：中
...
- **P2-4 嵌套块距离 fallback**：`calculateBlockDistance` 在 `toIndex===undefined`（块不在 `indexMap` 中，如嵌套块）时回退 `fromIndex+1`（ripple.ts:213）。DESIGN.md §4.3.5 标注为 v1 简化方案。要支持递归渐淡需设计决策，低优先级。
